...
class music_player:
	def __init__(self, master):
		...
		self.sidemenubar_treeview = ScrolledTreeView(self.sidemenubar_frame, custom_insert=False)
	...
	def add_folder_dialog(self):
		...
		if 'Gtk' not in sys.modules:
			Gdk.threads_add_idle(GLib.PRIORITY_DEFAULT, run_dialog_gtk, None)
			Gtk.main()
			self.add_folder(folder[0])
		else:
			folder = tkFileDialog.askdirectory(title="Please choose a folder")
	...
	def refresh_treeviews(self, tree='all', sort_by='Track Number'):
		def refresh_genre():
			try:
				...
				id = 'I001'
			self.genre_treeview.delete(*self.genre_treeview.get_children())
			self.genre_treeview.insert('', 'end', text='All Albums')
		...
		def refresh_artist():
			try:
				...
				id = 'I001'
			self.artist_treeview.delete(*self.artist_treeview.get_children())
			self.artist_treeview.insert('', 'end', text='All Albums')
		...
		def refresh_album():
			try:
				...
				id = 'I001'
			self.album_treeview.delete(*self.album_treeview.get_children())
		...
		def refresh_music(sort_by):
			self.music_treeview.delete(*self.music_treeview.get_children())
	...
	def add_m3u_dialog(self):
		...
		if 'Gtk' not in sys.modules:
			...
			m3u = tkFileDialog.askopenfilename(title="Please choose a .m3u file", filetypes = (("m3u files","*.m3u"),("all files","*.*")))
			self.add_m3u(m3u)
		...
		if os.path.isfile('cache.txt'):
			with open('cache.txt', 'r') as f:
				for line in f:
					...
					print('line=', line[0])
					if line[0] == 'artists':
						self.artists = collections.OrderedDict({key: collections.OrderedDict(value) for key, value in ast.literal_eval(line[1]).items()})
					elif line[0] == 'albums':
						self.albums = collections.OrderedDict({key: collections.OrderedDict(value) for key, value in ast.literal_eval(line[1]).items()})
					elif line[0] == 'genres':
						self.genres = collections.OrderedDict({key: collections.OrderedDict(value) for key, value in ast.literal_eval(line[1]).items()})
	...
	def radio_refresh_treeviews(self, stations='all'):
		self.radio_station_treeview.delete(*self.radio_station_treeview.get_children())
